...
from nti.app.products.badgr.interfaces import IBadgrIntegration,\
    IBadgrInitializationUtility, InvalidBadgrIntegrationError
...
@view_config(route_name='objects.generic.traversal',
             ...
             name=REL_AUTH_BADGR)
class BadgrAuthView(AbstractAuthenticatedView):
    """
    ...
    def __call__(self):
        ...
        auth_keys = OAuthKeys(APIKey=u'ABC',
                              SecretKey=u'123')
        ...
        auth_url = 'https://staging.badgr.io/auth/oauth2/authorize'
...
class BadgrAuth2(AbstractAuthenticatedView):
    ...
    def _create_auth_integration(self, access_data):
        ...
        try:
            intialization_utility.initialize(auth_integration)
        except InvalidBadgrIntegrationError:
            raise_error({'message': _(u"Invalid Badgr access."),
    ...
    def __call__(self):
        ...
        #auth_keys = component.getUtility(IOAuthKeys, name="badgr")
        from nti.common.model import OAuthKeys
        auth_keys = OAuthKeys(APIKey='ABC',
                              SecretKey='123')
        try:
            data = {'code': code,
                    'grant_type': 'authorization_code',
